<template>
    ...
    <div class="max-w-screen-xl mx-20 bg-white shadow sm:rounded-lg flex justify-center flex-1">
        <div class="flex-1 bg-indigo-100 text-center hidden lg:flex">
          <img src="../../../public/img/dashboard-meet.svg" class="m-12 xl:m-16 bg-no-repeat" />
        ...
        <div class=" flex flex-col items-center">
          ...
          <div class="w-full flex-1">
            ...
            <div class="p-1 w-full">
                ...
                <span class="w-full text-red-500" v-if="errors.firstname">{{errors.firstname[0]}}</span>
                <input class="w-full bg-gray-100 rounded border border-gray-400 focus:outline-none focus:border-indigo-500 text-base px-4 py-2" placeholder="First Name" type="text" v-model="form.firstname" >
            </div>
            <div class="p-1 w-full">
                <label class="w-full" for="name">Last Name</label>
                <span class="w-full text-red-500" v-if="errors.lastname">{{errors.lastname[0]}}</span>
                <input class="w-full bg-gray-100 rounded border border-gray-400 focus:outline-none focus:border-indigo-500 text-base px-4 py-2" placeholder="Last Name" type="text" v-model="form.lastname" >
            </div>
            <div class="p-1 w-full">
                <label for="email">Your e-mail</label>
                <input class="w-full bg-gray-100 rounded border border-gray-400 focus:outline-none focus:border-indigo-500 text-base px-4 py-2" placeholder="Email" type="email" v-model="form.email">
            ...
            <div class="p-1 w-full">
             ...
             <div class="p-1 w-full">
              ...
              <button @click.prevent="saveForm" type="submit"  class="mt-5 tracking-wide font-semibold bg-indigo-500 text-gray-100 w-full py-4 rounded-lg hover:bg-indigo-700 transition-all duration-300 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none">
    ...
    methods:{
        saveForm(){
            axios.post('/api/register', this.form).then(() =>{
                //console.log('saved');
                //this.$swal('Stream Stat Account', 'Account created successful', 'OK').then(this.$router.push({ path: 'login'}));
                this.$swal({ title: "StreamLab Stat Account",
                ...
                this.$swal('StreamLab Stat Account',error.response.data.errors);
            })
        }
    }
}
